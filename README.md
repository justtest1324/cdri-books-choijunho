...
| 상세 검색 | 제목/저자명/출판사 조건 검색 (팝오버 — 바깥 클릭·Esc로 닫힘, 다시 열면 활성 조건 유지). 전체 검색과 상호 배타 — 한쪽 실행 시 다른 쪽 조건 초기화 |
...
2. **원칙이 문서가 아니라 도구로 강제됨** — 데이터/계산/액션 분리·레이어 경계(컴포넌트는 api를 import 못 함)·훅 단일 책임이 oxlint 규칙과 검증 스크립트로 걸립니다. `npm run check` 한 번으로 타입·린트·컨벤션·테스트 63개가 검증됩니다.
3. **재사용 컴포넌트 설계** — 검색 결과와 찜 목록이 동일한 `BookList`를 사용하고, 타이포·컬러는 Figma 실측 토큰을 감싼 `Text`·`Button`으로만 사용합니다.
4. **실데이터 엣지 처리** — 카카오 API의 `sale_price: -1`(할인 없음), 빈 `thumbnail`(플레이스홀더), `page ≤ 50` 상한, 401/429/네트워크 오류별 안내와 재시도(단, 인증·쿼터 오류는 재시도 제외)를 처리했습니다. QA 중 **서로 다른 두 책이 같은 ISBN을 공유하는 실사례**를 발견해, 도서 식별 키를 isbn이 아닌 url의 bookId 기반(`getBookKey`)으로 재설계했습니다.
5. **성능·시각 안정성** — React Query 캐싱, 이미지 lazy loading, 무한 스크롤(IntersectionObserver), 빌드 타임 CSS, **폰트 셀프호스팅**(FOUT 제거), `scrollbar-gutter`(스크롤바 레이아웃 시프트 제거). 프로젝트 전체에서 `useEffect`는 IntersectionObserver 등록 1곳뿐입니다.
