...
# 1. localStorage 접근은 src/lib/ 에서만
hits=$(grep -rn "localStorage" src --include='*.ts' --include='*.tsx' -l 2>/dev/null | grep -v "^src/lib/" || true)
if [ -n "$hits" ]; then
...
# 2. fetch 호출은 src/api/ 에서만
hits=$(grep -rn "fetch(" src --include='*.ts' --include='*.tsx' -l 2>/dev/null | grep -v "^src/api/" || true)
if [ -n "$hits" ]; then
